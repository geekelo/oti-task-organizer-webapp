...
export default class PrintMe {
  ...
  removeBtnListener(e) {
    ...
    this.addedTasks = stored.map((item) => new TaskList(item.id, item.completed, item.description));
  ...
  checkStorage() {
    const stored = JSON.parse(localStorage.getItem('storedTasks'));
    if (stored) {
        this.addedTasks = stored.map((task) => new TaskList(task.id, task.completed, task.description));
        this.displayTasks();
    }
  }
